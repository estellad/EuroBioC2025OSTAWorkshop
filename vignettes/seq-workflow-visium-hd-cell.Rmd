...
Spatial transcriptomics (ST) data with cell types annotated can reveal key disease characteristics with neighbourhood enrichment and spatial statistics. In this instructor-led live demo, we analyse Visium HD data segmented to cells, demonstrating use of `SpatialFeatureExperiment` class in R to import, visualize, quality control, label transfer to annotate the data from a single-cell reference, spatial domain identification, marker gene selection, and spatial localisation of cells in tumor microenvironment. The complete analysis offered by existing tools highlights the ease with which researchers can turn the raw counts from a Visium HD experiment into biological insights using Bioconductor and CRAN. The complete workflow is available at <TODO> .
...
Participants can watch the live demo, or may prefer to follow the demonstration by bringing their laptops along. To follow the analysis on their own laptops, participants need to install the `EuroBioC2025OSTAWorkshop` workflow by running:

```{r}
BiocManager::install("EuroBioC2025OSTAWorkshop")
```

The relevant data are stored on Open Science Framework, and can be downloaded and cached on your laptop through `r BiocStyle::Biocpkg("OSTA.data")`:
...
We also prepared a reader for Visium HD segmented output at `./vignettes/utils.R`.
