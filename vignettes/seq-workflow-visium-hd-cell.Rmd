...
-   Perform spatial analysis in a tissue slice
...
<!-- Behind the scenes, we start with reading in cell segmentation as a `sf` object, scaling factor `.json` file, and an image `.png` file. We will use high resolution image, as it gives expected H&E details under segmented cell masks. The filtered count matrix `.h5` is read in as a `SingleCellExperiment`. Then, we construct a `SpatialExperiment` object with the above information, where the cell centroids were derived from the `sf` object and used as spatial coordinates. Next, we coerce the `SpatialExperiment` to a `SpatialFeatureExperiment`. The `sf` polygons are then stored in `colGeometries()`. To match the polygons with the orientation of the H&E, we had to flip the y-axis of these geometreis and the spatialcoordinates. Finally, we prepare the scaled coordinates in `colData()` for easier subsetting. 

Update: As of January 2026, pull requests branched from this reader have been merged into the VisiumIO and SpatialFeatureExperiment packages, enabling Visium HD cell-segmented data to be read as SpatialExperiment and SpatialFeatureExperiment objects, respectively. 
-->
...
Get cell-level QC metrics for visualization with `r BiocStyle::Biocpkg("scuttle")`.
...
`r BiocStyle::Biocpkg("SpotSweeper")` determines outliers based on low log-library size, few uniquely detected features, or a high mitochondrial fraction compared to their surrounding neighborhood:
...
We then synchronize the deconvolution labels back into our `vhdsub` `SpatialFeatureExperiment` object by matching cell ids to keep everything aligned.
...
Individual biomarkers identified in the DGE step may or may not have phenotypic relevance. Instead, we can quantify gene sets that are known to orchestrate biological functions or pathways, such as metabolic activity, cell cycle progression, and cell death, at the single-cell level. 

MSigDB provides a programmatic interface to the [Molecular Signatures Database (MSigDB)](https://www.gsea-msigdb.org/gsea/msigdb) (@subramanian2005gene), one of the largest collections of molecular signatures and pathways. We start with retrieving the human hallmark database. 
...
Next, we score these gene sets with `r BiocStyle::Biocpkg("AUCell")` (@aibar2017scenic), a rank-based approach that (i) ranks genes for every cells, and (ii) compute area under the recovery curve (AUC) values for each gene set.
...
A heatmap can be used to check which signatures are highly expressed in which cell types. To diversify the visualization, here we summarize the top 2 signatures per cell type, and display their scaled average enrichment score in a waterfall plot. 
...
p53 is a well-known pathway related to tumor cell growth (@vogelstein2000surfing), and its enrichment score can be visualized spatially.
...
## Shape statistics
Because cell segmentation polygons are available for each cell, we can derive a range of shape metrics (@gunz2025analysis).
```{r shape-metrics}
...
tshme |>
  ...
  guides(fill = "none")
```
We observer more uniform distribution in `Compactness`, while more variability in `fibreLength` across cell types. 
...
What makes spatial transcriptomics more powerful than scRNA-seq is its integration with spatial statistics. There are two types of measures for spatial association: global and local.

### Global spatial association

We next quantify spatial proximity between annotated cell types. The `getPairwise()` function from the `r BiocStyle::Biocpkg("spicyR")` package computes the K-cross function across all pairs of classes, summarizing whether cell type A tends to appear within distance `r` of cell type B more often than expected under spatial randomness. Here we use a radius of 200 units (\~50 microns) to capture immediate neighborhood effects at approximately one to two cell diameters.
...
We can re-visit the spatial composition of cell types, annotated by `RCTD` above, to build intuition for these statistics. This helps confirm that any strong pairwise signals are visible in the image and not driven by a handful of cells.

### Local indicator of spatial association - LISA
Visium HD cell-level data can be modeled as a point pattern process (@emons2025harnessing). Different than the previous global average measure, local indicators of spatial association (LISA) accounts for the local contribution of each cell (@anselin1995local). First, we construct the plotting data frame that contains LISA Ripley's K value for only Tumor cells.
...
Plot the LISA Ripley’s K value for each tumor cell, colored by its value at a radius of 700 μm. Curves above the gray line representing complete spatial randomness (CSR) indicate highly clustered tumor cells.
```{r lisa-plot, fig.height=4, fig.width=4.5, warning=FALSE, message=FALSE}
ggplot(plt_df, aes(r, value, group=curve, col=sel)) +
  geom_line() +
...
This is where `Kontextual` from the `r BiocStyle::Biocpkg("Statial")` package comes in. `Kontextual` extends beyond pairwise enrichment by creating a contextual null model. Instead of comparing two cell types against a fully random distribution, it compares them against what we would expect given the observed mixture of other neighboring cell types. In practice, this means that `Kontextual` is testing whether a particular association is stronger (or weaker) than would be expected given the local cellular environment.
...
We can then inspect radii-dependent curves (for example Myeloid around Tumor) to confirm at which scales the effect appears, which is often informative for processes like invasion fronts or peritumoral niches.
...
```
Because immune cells are already densely packed, Kontextual identifies myeloid cells as being particularly close to tumor cells beyond a 50-pixel radius. In contrast, when immune context is not considered, the L-function suggests that myeloid cells and tumor cells are not spatially close.
...
The previous output is a ranked list of genes, highlighting markers that change as Myeloid cells are near tumor cells. One of the top genes on this list is *SPP1* which is known to be expressed in tumor-associated macrophages. We can visualize this relationship using the `plotStateChanges()` function.
